...
import { ArrowLeft, Droplet, Heart, MapPin, Mail, Phone, Calendar, AlertCircle, Loader2, MessageCircle } from 'lucide-react';
...
export default function DonorProfile() {
  ...
  useEffect(() => {
    const fetchDonor = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/users/${donorId}`);
        if (!res.ok) {
